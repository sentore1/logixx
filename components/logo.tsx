...
export const Logo = ({ className, uniColor, isScrolled }: { className?: string; uniColor?: boolean; isScrolled?: boolean }) => {
    return (
        <Image 
            src="/New folder/logo.png" 
            alt="Flash Logix Logo" 
            width={200} 
            height={75} 
            className={cn(
                'h-12 w-auto transition-all duration-300',
                !uniColor && !isScrolled ? 'brightness-0 invert' : '',
                className
            )} 
        />
    )
